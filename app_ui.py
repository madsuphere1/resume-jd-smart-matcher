...
def extract_text_from_pdf(file):
    ...
    # Configure Tesseract path when needed
...
if st.button("🚀 Analyze Resume", type="primary", use_container_width=True):
    ...
    status_placeholder.info("⏳ Setting up your Resume ↔ JD Matcher and AI models... This may take a moment.")
    ...
    # Recompute base score with user-configured weights
    base_score = (
    ...
    if enable_llm and "llm_alignment_score" in result and result["llm_alignment_score"] is not None:
        final_score = (base_score * 0.60) + (result["llm_alignment_score"] * 0.40)
        ai_match_display = f"<span class='sub-score'>🤖 AI Match: <strong>{result['llm_alignment_score']*100:.0f}%</strong></span>"
    else:
    ...
    if result["explanations"]:
        ...
        for exp in result["explanations"]:
            score_val = exp.get("llm_score", 0.5)
            verdict_icon = "✅" if score_val >= 0.8 else ("⚠️" if score_val >= 0.5 else "❌")
            
            with st.expander(
                f"{verdict_icon} {exp['jd_text'][:80]}... (AI Match: {score_val*100:.0f}%)"
            ):
                st.markdown(f"**JD Requirement:** {exp['jd_text']}")
                st.markdown(f"**Resume Section:** {exp['resume_text'][:300]}")
                st.markdown(f"**AI Match Score:** `{score_val*100:.0f}%`  \n**AI Analysis:** {exp['explanation']}")

    # ══════════════════════════════════════════════════════════════════
    # DETAILED METRICS (HIDDEN IN EXPANDER)
    # ══════════════════════════════════════════════════════════════════
    
    st.markdown("---")
    with st.expander("🔎 Explore More Details (Section Grid, Skill Analysis, RAG)", expanded=False):
        ...
        for i, match in enumerate(sorted_matches):
            ...
            <div class="{card_class}">
                ...
                <small>Semantic Match: <strong>{score:.3f}</strong></small>
            </div>
            """, unsafe_allow_html=True)
